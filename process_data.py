...
3. find_oov()
4. generate_oov_vectors()
5. remove_no_gold_label_samples()
...
def find_oov(db_name):
    """Determine OOV tokens in a dataset.
    ...
    GloVe vectors provided by SpaCy, and returns them in a dictionary
    of structure oov[collection_name][doc_id] = token_text. This
    dictionary will be saved as a pickle named oov_db_name.pkl.

    Make sure generate_friendly_ids has been performed first because
        ...
        for doc in repository.find_all():
            ...
            for token in premise_doc:
                if np.array_equal(token.vector, zero_vector):
                    oov[collection_name][doc['id']] = token.text
                    got_one = True
            ...
            if got_one:
                count += 1
        print('%s samples with OOV in %s.%s' %
              (count, db_name, collection_name))

    print('Completed successfully.')
    util.save_pickle(oov, 'oov_%s.pkl' % db_name)
    return oov


def generate_oov_vectors(oov):
    """Create random vectors for OOV tokens.

    Args:
      oov: dictionary of oov tokens generated by find_oov(db_name),
        of the form [oov][collection_name][id] = token_text.
    Returns:
      None.
    Raises:
      None.
